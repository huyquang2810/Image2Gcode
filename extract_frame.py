...
#     else:
#         # Nhỏ hơn → không resize, giữ nguyên nét gốc
#         new_width, new_height = original_width, original_height
#
#     # === Tạo nền trắng và canh giữa ===
...
#     img_path = os.path.join(base_dir,img_path)
    base_dir = os.path.dirname(os.path.abspath(__file__))
    full_path = os.path.join(base_dir, "input_framePicture", img_name)
    ...
    # === PHẦN BỊ TẮT TẠM THỜI: TÌM VÀ CẮT KHUNG ===
    ...
    # ✅ Resize ảnh đã lọc màu, KHÔNG cắt khung
    # pil_image = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    # resized = resize_with_padding(pil_image, target_size=(320, 240))
    # resized = resized.convert("RGB")
    #
    # inside_path = os.path.join(base_dir, output_inside_dir, img_name)
    # resized.save(inside_path, dpi=(1280, 960), quality=100, progressive=True)
    #
    # print(f"✅ OK (NO FRAME CROP): {os.path.basename(full_path)} → lọc màu + resize")
    # return None

if __name__ == "__main__":
    input_folder = "input_framePicture"
